<header class="bg-white shadow" x-data="{ mobileMenuOpen: false, searchDropdownOpen: false, langDropdownOpen: false }">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-4 lg:py-6">
            <div class="flex items-center">
                <a href="/" class="flex-shrink-0">
                    <img class="h-10 w-auto sm:h-10" width="107" height="40" src="{{ asset('images/logo.svg') }}" alt="{{ config('app.name') }} logo"/>
                </a>
            ...
            <div class="flex items-center space-x-4">
                <!-- Language Switcher -->
                <div x-data="{ langDropdownOpen: false }" class="relative">
                    <button @click="langDropdownOpen = !langDropdownOpen" class="text-base font-medium text-gray-500 hover:text-gray-900 flex items-center">
                        {{ strtoupper(app()->getLocale()) }} <!-- Shows current language -->
                        <svg class="ml-1 h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd" />
                        </svg>
                    </button>
                    <div x-show="langDropdownOpen" x-cloak @click.away="langDropdownOpen = false" class="absolute z-50 right-0 mt-2 w-28 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5">

                        <a href="{{ route('lang.switch', 'az') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                            {{ __('AZ') }}
                        </a>
